...
exports.bookinstance_delete_get = function(req, res, next) {
    ...
    .exec(function (err, bookinstance) {
        if (err) { return next(err); }
        if (bookinstance==null) { // No result, redirect to book instances list
...
exports.bookinstance_delete_post = function(req, res, next) {
    ...
    BookInstance.findByIdAndRemove(req.body.bookinstanceid, function deleteBookInstance(err) {
        if (err) { return next(err); }
        // Success redirect to list of Book Instances
        res.redirect('/catalog/bookinstances');
    });
...
exports.bookinstance_update_post = [
    ...
    (req, res, next) => {
        ...
        // Create a BookInstance object with the escaped/trimmed data and current id
        var bookinstance = new BookInstance(
            {
                book: req.body.book,
                imprint: req.body.imprint,
        ...
        if (!errors.isEmpty()) {
            // There are erros so render the form again, passing sanitized values and errors
            Book.find({}, 'title')
                .exec(function (err, books) {
                    if (err) { return next(err); }
                    // Successful, so render
